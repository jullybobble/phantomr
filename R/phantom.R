...
#' Read javascript-generated HTML pages.
#'
...
#'
#' @param url [character vector] the url whose HTML to download and parse
#' @param ... additional parameters passed to \code{\link[xml2]{read_html}}
#' @param .phantomjs a PhantomJS process as returned by \code{\link[webdriver]{run_phantomjs}()}
#' @param .phantom_session a \code{\link[webdriver]{Session}}, for exampled as returned by
#'                         \code{Session$new(port = run_phantomjs()$port)}
#' @param .simplify
#'
#' @return The resulting HTML document normalised to valid XML as by \code{\link[xml2]{read_html}}.
#'
#' @seealso xml2 read_html
#' @seealso \url{https://github.com/rstudio/webdriver}
#'
#' @examples
#'
#' library(rvest)
...
#' @export
read_html_js <- function(url, ..., .phantomjs = NULL, .phantom_session = NULL, .simplify = FALSE) {
  if(is.null(.phantom_session)) {
    if(is.null(.phantomjs)) {
      .phantomjs <- run_phantomjs()
    }
  ...
  html <- map(url, ~ {
    .phantom_session$go(.)
    read_html(.phantom_session$getSource(), ...)
  })
  if(.simplify && length(html) == 1) {
    html[[1]]
  } else {
    html
  }
